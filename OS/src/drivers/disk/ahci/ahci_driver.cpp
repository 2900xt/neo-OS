...
namespace disk
{
    ...
    {
        gpt_data = (filesystem::gpt_part_data *)kernel::allocate_pages(4);
        kernel::map_pages((uint64_t)gpt_data, (uint64_t)gpt_data, 4);
        filesystem::gpt_part_table_hdr *gpt_hdr = &gpt_data->hdr;
        ...
        for (lba = gpt_hdr->lba_part_entry_arr; lba < gpt_entry_sectors + gpt_hdr->lba_part_entry_arr; lba++)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                currentEntry++;
                if (gpt_data->entries[currentEntry].starting_lba == 0)
                    return;
            }
        }
    }

    filesystem::gpt_part_data *AHCIDevice::get_gpt()
    {
        return gpt_data;
    }
