...
static uint32_t get_next_cluster(FS::FAT_partition* partition, int current_cluster)
{
...
static fat_dir_entry* copy_dir_entry(fat_dir_entry* entry)
{
    ...
    memcpy(copy, entry, sizeof(fat_dir_entry));
    return copy;
}
...
static fat_dir_entry* search_directory(FAT_partition* partition, fat_dir_entry* directory, const char *filename)
{
    ...
    while(true)
    {
        ...
        if(std::strcmp(filename, current_entry->dir_name, 11))
        {
...
const char* filename_to_fat(std::string& formal_filename)
{
    char* fat_filename = new char[MAX_FILENAME_LENGTH];
    memset_8(fat_filename, MAX_FILENAME_LENGTH, ' ');
    ...
    for(int i = 0; i < MAX_FILENAME_LENGTH; i++)
    {
        ...
        fat_filename[i] = std::toUpper(srcChar);
        srcIndex++;
    }
...
fat_dir_entry* get_file_entry(FAT_partition* partition, std::string* filepath)
{
    ...
    int dir_levels;
    std::string** path = filepath->split('/', &dir_levels);
...
FS::FAT_partition* mount_part(DISK::rw_disk_t *device, int part_num, VFS::File* folder)
{
    ...
    std::string path{"/bin/kernel.elf"};
