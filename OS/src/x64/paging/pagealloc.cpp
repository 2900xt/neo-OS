...
void initialize_allocator(void *base, void *end)
{
    ...
    klogf(LOG_IMPORTANT, "Starting Page Allocater at Address 0x%x\tLength: 0x%x\n", allocator_base, allocator_sz);
...
void* next_page(void)
{
    ...
    void *addr = (void*)((current_index * page_sz) + allocator_base);

    map_page((uint64_t)addr, (uint64_t)addr);
