...
//Bitmap is gonna be located at the start of the heap, with the bitmap being the size of the heap block count
    ...
    klogf(LOG_IMPORTANT ,"Starting kernel heap at: 0x%x\tWith length: 0x%x\n", heapOffset, heapBlksize * heapBlkcount);
    
//Start the page frame allocator, round up addresses to 4K

    largestFreeRegion->base = (0x1000 - (largestFreeRegion->base % 0x1000)) + largestFreeRegion->base;
    largestFreeRegion->length = largestFreeRegion->length - (largestFreeRegion->length % 0x1000);
    AMD64::initialize_allocator((void*)largestFreeRegion->base, (void*)(largestFreeRegion->base + largestFreeRegion->length));
