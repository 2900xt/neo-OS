...
struct bios_param_block {
    ...
    [[gnu::deprecated]]
    uint16_t total_sector_count_16;  // If set to 0, there are more than 65535
                                     // sectors and the actual value is in the
                                     // large sector counts
    uint8_t media_type;
    [[gnu::deprecated]]
...
int write_cluster_chain(FAT_partition* partition, fat_dir_entry* file_entry, void* buffer, uint64_t size);
}  // namespace filesystem